...
void find_word(int thread,std::vector<std::string> assignedLines, int begin, int end, Client& c, std::mutex& access_balance, std::mutex& access_result);
...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i<NCLIENTS; i++){
        vClients.push_back(Client(i,WORDS[(rand()%WORDS.size())],rand()%50+1,(rand()%2)));
    }
...
void create_threads(std::string file, Client& c, std::mutex& access_balance){
    ...
    for (int i = 0; i < NTHREADS; i++) {
        ...
        vThreads.push_back(std::thread(find_word, i, assignedLines[i], begin, end, std::ref(c), std::ref(access_balance), std::ref(access_result)));
    }
...
void find_word(int thread,std::vector<std::string> assignedLines, int begin, int end, Client& c,std::mutex& access_balance, std::mutex& access_result){
    ...
    for(std::size_t nLine = 0; nLine < assignedLines.size(); nLine++){
        ...
        std::cout<<YELLOW<<"Se ejecuta acaso "<<line.size()<<std::endl;
...
std::vector<std::string> splitLine(std::string line){
    ...
    std::cout<<YELLOW<<"Split line: "<<result.size()<<std::endl;
